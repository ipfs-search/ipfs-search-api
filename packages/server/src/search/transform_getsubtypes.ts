...
export default function GetSubtypeFromQuery(q: SearchQuery): SearchQuery {
  if (![DocType.File, "any"].includes(q.type) || q.subtypes?.length) {
    return q;
  }

  debug("Getting subtype from query");
